...
from binaryHeap import RevGBinaryHeap
import time
import pickle
import copy
...
def isClear(x, y):
    ...
    if ((y >= 0 and y < len(a[0])) and (x >= 0 and x < len(a))):
        # print(closed)
        # print((x, y), (x,y) in closed.keys())
        if ((not block.get((x, y)))) and not closed.get((x, y)):
            # print("returning true", (x, y))
            # print(not closed.get((x, y)))
            return True
    return False
...
def findAstar(point):
    ...
    """
    global closed, seen, gval, hval, block, counter
    closed.clear()
    getBlockers(point)
    q1 = RevGBinaryHeap()
    h_val = calcHval(point)
    cost = h_val + gval[(point.x, point.y)]
    # print("Cost of ", point, "is", cost)
    ...
    while q1.size > 0:
        ...
        if (isClear(current.x+1, current.y)):
            ...
            h_val = calcHval(s1)
            cost = h_val + gval[(current.x, current.y)]+1
